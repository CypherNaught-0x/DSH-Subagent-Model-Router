window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BetterSidebarSubagentTab({ ctx, scope, visible }) {
      ...
      const catalogs = snapshot?.subagentsByParent ?? {}
      const summaries = snapshot?.byId ?? {}
      const catalog = rootId === undefined ? undefined : catalogs[rootId]
      const [expanded, setExpanded] = React.useState(() => new Set())
      const [now, setNow] = React.useState(() => Date.now())
      const observed = React.useRef(new Set())
      const observedRoot = React.useRef(null)
      ...
      const t = (key, vars = {}) => {
        const labels = {
          ...
          'diagnostic.corrupt': 'Corrupt session',
          'diagnostic.unsupported': 'Unsupported session',
          ...
          'count.total.one': '1 sub-agent',
          'count.total.other': `${vars.count ?? 0} sub-agents`,
          'count.running.one': '1 sub-agent running',
          'count.running.other': `${vars.count ?? 0} sub-agents running`,
        }
      ...
      const presentedCatalog = rootId === undefined
        ? undefined
        : stats.count > 0 && (catalog === undefined || (catalog.state === 'ready' && catalog.entries.length === 0))
          ? { entries: [], parentAvailable: false, state: 'loading', error: null }
          : catalog
      const observe = (id, open) => {
        if (open) observed.current.add(id)
        else observed.current.delete(id)
        setCatalogObserved(sessions, id, open)
      }
      const closeBranch = (id) => {
        ...
        const visit = (parent) => {
          ...
          closing.add(parent)
          for (const entry of catalogs[parent]?.entries ?? []) if (entry.kind === 'child') visit(entry.id)
        }
        visit(id)
        for (const parent of closing) observe(parent, false)
        setExpanded(new Set([...expanded].filter((value) => !closing.has(value))))
      }
      const toggleBranch = (id) => {
        if (expanded.has(id)) closeBranch(id)
        else { setExpanded(new Set(expanded).add(id)); observe(id, true) }
      }
      React.useEffect(() => {
        if (!visible || rootId === undefined) return undefined
        const rootChanged = observedRoot.current !== rootId
        observedRoot.current = rootId
        observe(rootId, true)
        // Expansion belongs to a root session. Do not carry old-root branch
        // observations across a session switch; the cleanup above closes them.
        if (!rootChanged) for (const id of expanded) if (id !== rootId) observe(id, true)
        return () => {
          for (const id of observed.current) setCatalogObserved(sessions, id, false)
      ...
      if (presentedCatalog.state === 'ready' && presentedCatalog.entries.length === 0 && stats.count === 0) {
    ...
    function apply(ctx) {
      ...
      ctx.inject(['betterSidebar'], (sidebarCtx) => {
        ...
        sidebarCtx.effect(() => betterSidebar.registerTab({
          ...
          order: 31,
          single: true,
          component: (props) => h(BetterSidebarSubagentTab, props),
        }))
      })
