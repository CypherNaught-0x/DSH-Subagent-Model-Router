window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const catalogObservationCounts = new WeakMap()
    ...
    const BETTER_SIDEBAR_CHIP_ATTRIBUTE = 'data-dsh-subagent-model-route'
    const SIDE_CHAT_LABEL_PREFIX = 'Side: '
    ...
    function catalogChildren(snapshot, parentSessionId) {
      const byId = snapshot?.byId ?? {}
      const entries = snapshot?.subagentsByParent?.[parentSessionId]?.entries
      if (!Array.isArray(entries)) return []
      return entries.filter((entry) => entry.kind === 'child' && !(entry.label?.startsWith(SIDE_CHAT_LABEL_PREFIX) ?? false)).map((entry) => ({
        entry,
        label: entry.label ?? byId[entry.id]?.displayTitle ?? entry.id,
      }))
    }

    /** Resolve Better Sidebar's visible tree rows to exact catalog child ids. */
    function betterSidebarTaskRowAssignments(snapshot, rows) {
      const rootId = mainSessionId(snapshot)
      if (rootId === undefined) return []
      const byId = snapshot?.byId ?? {}
      const parentAtLevel = [rootId]
      const nextChildByParent = new Map()
      const assignments = []

      for (const row of rows) {
        const level = Number(row.level)
        if (!Number.isSafeInteger(level) || level < 0) continue
        if (level === 0) {
          parentAtLevel[0] = rootId
          parentAtLevel.length = 1
          continue
        }
        const parentSessionId = parentAtLevel[level - 1]
        parentAtLevel.length = level
        if (parentSessionId === undefined || row.disabled) continue
        const children = catalogChildren(snapshot, parentSessionId)
        const start = nextChildByParent.get(parentSessionId) ?? 0
        let index = children.findIndex((child, childIndex) => childIndex >= start && child.label === row.label)
        if (index < 0) index = children.findIndex((child, childIndex) => childIndex >= start && byId[child.entry.id]?.displayTitle === row.label)
        if (index < 0) continue
        const child = children[index]
        nextChildByParent.set(parentSessionId, index + 1)
        parentAtLevel[level] = child.entry.id
        const summary = byId[child.entry.id]
        if (summary !== undefined) assignments.push({ row: row.element, label: row.labelElement, summary })
      }
      return assignments
    }

    function labelTextWithoutModelChip(label) {
      return Array.from(label.childNodes).filter((node) => {
        return node.nodeType !== 1 || !node.hasAttribute(BETTER_SIDEBAR_CHIP_ATTRIBUTE)
      }).map((node) => node.textContent ?? '').join('').trim()
    }

    function taskRowDescriptors(tree) {
      return Array.from(tree.querySelectorAll('[role="treeitem"]')).flatMap((row) => {
        const content = Array.from(row.children).find((child) => {
          return child.tagName === 'SPAN' && Array.from(child.children).filter((nested) => nested.tagName === 'SPAN').length >= 2
        })
        const labelElement = content?.firstElementChild
        if (labelElement === null || labelElement === undefined || labelElement.tagName !== 'SPAN') return []
        return [{
          element: row,
          labelElement,
          label: labelTextWithoutModelChip(labelElement),
    ...
    function styleBetterSidebarModelChip(chip) {
      Object.assign(chip.style, styles.modelChip, {
        display: 'inline-block',
        maxWidth: 'min(160px, 45%)',
        marginLeft: '6px',
        padding: '0 6px',
        fontSize: '10px',
        lineHeight: '16px',
    ...
    function mountBetterSidebarTaskModelChips(ctx, sessions) {
      ...
      ctx.effect(() => {
        ...
        let disposed = false
        let scheduled = false
        const managedChips = new Set()
        const ariaRows = new Map()

        const restoreRowAria = (row, record) => {
          if (row.getAttribute('aria-label') !== record.augmented) return
          if (record.base === null) row.removeAttribute('aria-label')
          else row.setAttribute('aria-label', record.base)
        }
        ...
        const reconcile = () => {
          ...
          for (const root of roots) {
            rootObserver.observe(root, {
              ...
              characterData: true,
              attributes: true,
              attributeFilter: ['aria-label', 'aria-level', 'aria-disabled'],
            })
            for (const tree of root.querySelectorAll('[role="tree"]')) {
              const assignments = betterSidebarTaskRowAssignments(snapshot, taskRowDescriptors(tree))
              for (const assignment of assignments) {
                const route = modelRoute(assignment.summary)
                if (route === undefined) continue
                const identity = modelIdentity(route, configuredModelsSnapshot)
                let chip = assignment.label.querySelector(`[${BETTER_SIDEBAR_CHIP_ATTRIBUTE}]`)
                if (chip === null) {
                  chip = document.createElement('span')
                  chip.setAttribute(BETTER_SIDEBAR_CHIP_ATTRIBUTE, identity.fullRoute)
                  chip.setAttribute('aria-hidden', 'true')
                  styleBetterSidebarModelChip(chip)
                  assignment.label.append(chip)
                }
                managedChips.add(chip)
                if (chip.getAttribute(BETTER_SIDEBAR_CHIP_ATTRIBUTE) !== identity.fullRoute) chip.setAttribute(BETTER_SIDEBAR_CHIP_ATTRIBUTE, identity.fullRoute)
                if (chip.textContent !== identity.name) chip.textContent = identity.name
                if (chip.title !== identity.label) chip.title = identity.label
                activeChips.add(chip)

                const row = assignment.row
                const currentAria = row.getAttribute('aria-label')
                let record = ariaRows.get(row)
                if (record === undefined) {
                  record = { base: currentAria, augmented: null }
                  ariaRows.set(row, record)
                } else if (currentAria !== record.augmented) {
                  record.base = currentAria
                }
                const augmented = [record.base, `Model ${identity.label}`].filter(Boolean).join(' ')
                if (row.getAttribute('aria-label') !== augmented) row.setAttribute('aria-label', augmented)
                record.augmented = augmented
                activeRows.add(row)
              }
            }
          }

          for (const chip of managedChips) {
            if (activeChips.has(chip)) continue
            chip.remove()
            managedChips.delete(chip)
          }
          for (const [row, record] of ariaRows) {
            if (activeRows.has(row)) continue
            restoreRowAria(row, record)
            ariaRows.delete(row)
          }
        }
        ...
        // Better Sidebar mounts its semantic root directly under body. Observe
        // only that boundary globally; all high-churn tree work stays scoped to
        // the discovered sidebar root(s).
        bodyObserver.observe(document.body, { childList: true })
        const offList = list.subscribe(schedule)
        configuredModelListeners.add(schedule)
        schedule()
        return () => {
          disposed = true
          ...
          rootObserver.disconnect()
          offList()
          configuredModelListeners.delete(schedule)
          for (const chip of managedChips) chip.remove()
          for (const [row, record] of ariaRows) restoreRowAria(row, record)
          managedChips.clear()
          ariaRows.clear()
        }
      }, 'dsh-subagent-model-router: Better Sidebar Tasks model chips')
    }

    function setCatalogObserved(sessions, parentSessionId, open) {
      if (typeof sessions.setSubagentCatalogOpen !== 'function') return
      let counts = catalogObservationCounts.get(sessions)
      if (counts === undefined) {
        if (!open) return
        counts = new Map()
        catalogObservationCounts.set(sessions, counts)
      }
      const previous = counts.get(parentSessionId) ?? 0
      if (open) {
        counts.set(parentSessionId, previous + 1)
        if (previous === 0) sessions.setSubagentCatalogOpen(parentSessionId, true)
      } else if (previous === 0) {
        return
      } else if (previous === 1) {
        counts.delete(parentSessionId)
        if (counts.size === 0) catalogObservationCounts.delete(sessions)
    ...
    function CatalogRows({ parentSessionId, catalog, catalogs, summaries, configuredModels, expanded, level, now, sessions, closeCatalog, toggleBranch, t }) {
      const rows = []
    ...
    function SubagentCatalogAction({ sessionId, useSessions, sessions, t }) {
      ...
      const setCatalogOpenRef = React.useRef((parentSessionId, next) => setCatalogObserved(sessions, parentSessionId, next))
      setCatalogOpenRef.current = (parentSessionId, next) => setCatalogObserved(sessions, parentSessionId, next)
      ...
      const observeCatalog = (parentSessionId, next) => {
        ...
        setCatalogObserved(sessions, parentSessionId, next)
      }
      const closeAllCatalogs = () => {
        for (const parentSessionId of observedCatalogs.current) setCatalogObserved(sessions, parentSessionId, false)
    ...
    function mountConfiguredModelNames(ctx, connection, remote) {
      ctx.effect(() => {
        ...
        const offRemote = remote.$on('settings/document-updated', (namespace) => {
          if (namespace === SETTINGS_NAMESPACE) void load()
        })
    ...
    function createSettingsSection(ctx, connection, remote) {
      return function SubagentModelsSettings() {
        ...
        return h('section', { style: styles.section },
          ...
          h('p', { style: styles.intro }, 'Choose model routes for delegated work. Friendly names appear in subagent headers, catalogs, and Better Sidebar Tasks rows when that plugin is installed. Changes apply live from subagent-model-router in settings.yaml.'),
    ...
    function apply(ctx) {
      ...
      mountBetterSidebarTaskModelChips(ctx, sessions)
    ...
    module.exports.betterSidebarTaskRowAssignments = betterSidebarTaskRowAssignments
