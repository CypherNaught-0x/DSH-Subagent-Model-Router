...
function createOrchestrationTracker(ctx) {
  ...
  const updateRecord = (record, model, label, agent) => {
    ...
    if (record.agent === undefined && agent !== undefined) {
      record.agent = agent
      record.boundary = agent.session.events.length
    }
  ...
  const trackRecord = (state, childId, model, label, agent) => {
    if (state.disposed) return
    const existing = state.children.get(childId)
    if (existing !== undefined) {
      updateRecord(existing, model, label, agent)
      return
    }
    let resolve
    ...
    const record = {
      ...
      label,
      agent,
      boundary: agent?.session.events.length ?? 0,
      resolve,
      settled,
  ...
  ctx.on('subagent/start', function (info) {
    ...
    trackRecord(stateFor(parent), info.id, descriptor.agentModel, descriptor.label, child)
  })
  ...
  const reconcile = (records) => {
    ...
    for (const record of records) {
      ...
      if (current === record.agent) continue
      const events = record.agent.session.events.slice(record.boundary)
      settle(record, {
        stopReason: recoveredStopReason(events),
        lastAssistantMessage: recoveredAssistantOutput(events),
      })
      ctx.logger.warn(`${WAIT_TOOL_NAME}: recovered missed terminal event for subagent ${record.subagentId}`)
    }
  }
  ...
  return {
    ...
    track(state, childId, model, label) {
      trackRecord(state, childId, model, label, ctx.get('agents')?.get(childId))
    },
    finish(parent, state) {
      if (state.disposed) return
      state.starting -= 1
    ...
    async wait(parent, signal) {
      ...
      try {
        ...
        while (settlements === undefined) {
          ...
          try {
            const outcome = await awaitWithSignal(Promise.race([
              allSettled.then((value) => ({ kind: 'settled', value })),
              watchdog.promise.then(() => ({ kind: 'watchdog' })),
            ]), signal)
            if (outcome.kind === 'settled') settlements = outcome.value
...
function registerWaitTool(ctx, tracker) {
  ...
  const tool = defineTool({
    name: WAIT_TOOL_NAME,
    description: `Wait for every outstanding continuable background subagent started by this agent, including standard delegations and those started through ${DELEGATION_TOOL_NAME}. Call this once after issuing all intended delegations; it blocks until they settle and returns their results, so do not poll.`,
    parameters: {},
    output: {
      schema: {
        type: 'array',
