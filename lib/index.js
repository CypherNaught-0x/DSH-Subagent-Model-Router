...
function createOrchestrationTracker(ctx) {
  ...
  const stateFor = (parentId) => {
    let state = states.get(parentId)
    if (state === undefined) {
      state = { children: new Map(), starting: 0, waiters: new Set(), waiting: 0 }
  ...
  const prune = (parentId, state) => {
    if (state.starting === 0 && state.children.size === 0 && state.waiting === 0) {
      states.delete(parentId)
    }
  }

  const settle = (record, info) => {
    if (record.settlement !== undefined) return
    const output = outputValueText(info.lastAssistantMessage ?? [])
    record.settlement = {
  ...
  return {
    ...
    finish(parentId, state) {
      state.starting -= 1
      startsInFlight -= 1
      notify(state)
      if (startsInFlight === 0) earlySettlements.clear()
      prune(parentId, state)
    },
    async wait(parentId, signal) {
      const state = stateFor(parentId)
      state.waiting += 1
      try {
...
function registerWaitTool(ctx, tracker) {
  if (ctx.tools.get(WAIT_TOOL_NAME) !== undefined) return
  ctx.tools.register(defineTool({
    name: WAIT_TOOL_NAME,
    ...
    output: {
      ...
      render: (_args, settlements) => [{
        ...
        text: settlements.length === 0
          ? '(no outstanding model-routed subagents)'
          : settlements.map((entry) => {
              const output = entry.output === undefined ? '' : `\n${entry.output}`
              return `${entry.subagentId} [${entry.stopReason}] ${entry.label} (${entry.model})${output}`
            }).join('\n\n'),
      }],
    },
    isConcurrencySafe: () => true,
    execute(_args, exec) {
      ...
      return tracker.wait(parent.id, exec.signal)
    },
  }))
}
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      if (backgroundEnabled && args.run_in_background !== false) {
        const trackingState = tracker.begin(parent.id)
        try {
          ...
          tracker.finish(parent.id, trackingState)
        }
      }
...
async function apply(ctx) {
  ...
  const mountCurrent = (provider) => {
    ...
    try {
      ...
      disposeTool = ctx.tools.register(createDelegationTool(ctx, tracker, current, provider, current.models))
    } catch (error) {
  ...
  ctx.systemPrompt.section({
    ...
    text: (context) => disposeTool === undefined || ctx.tools.get(DELEGATION_TOOL_NAME, context.scope) === undefined
      ? ''
