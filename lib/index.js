...
function createOrchestrationTracker(ctx) {
  ...
  const updateRecord = (record, model, label) => {
  ...
  const trackRecord = (state, childId, model, label) => {
    if (state.disposed) return
    const existing = state.children.get(childId)
    if (existing !== undefined) {
      updateRecord(existing, model, label)
      return
    }
    let resolve
  ...
  ctx.on('subagent/start', function (info) {
    ...
    trackRecord(stateFor(parent), info.id, descriptor.agentModel, descriptor.label)
  })
  ...
  return {
    ...
    track(state, childId, model, label) {
      trackRecord(state, childId, model, label)
    },
    finish(parent, state) {
      if (state.disposed) return
      state.starting -= 1
