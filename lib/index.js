...
const inject = ['tools', 'subagents', 'systemPrompt', 'skills', 'llm', 'settings']
...
function normalizeSettings(value = {}) {
  ...
  const toolName = value.toolName ?? 'subagent_model'
  ...
  if (toolName === CATALOG_TOOL_NAME || toolName === CONFIG_TOOL_NAME) {
    throw new Error(`dsh-subagent-dynamic-model: toolName must differ from ${CATALOG_TOOL_NAME} and ${CONFIG_TOOL_NAME}`)
  }
...
function renderConfigurationResult(value) {
  ...
  return `${value.status === 'updated' ? 'Updated' : 'Current'} subagent model settings.\nTool: ${settings.toolName}; backend: ${settings.subagentProvider}; max depth: ${settings.maxDepth}; background: ${String(settings.enableRunInBackground)}\n${routes}`
}
...
function registerConfigurationTool(ctx, settingsScope) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      tool_name: {
        type: 'string',
        description: 'Optional model-facing delegation tool name; omitted preserves the current value.',
      },
      max_depth: {
        type: 'integer',
    ...
    async execute(args) {
      ...
      const next = normalizeSettings({
        ...
        toolName: args.tool_name ?? current.toolName,
...
function registerSettingsRoute(ctx) {
  ...
  }), 'dsh-subagent-dynamic-model: settings web route')
}
...
async function apply(ctx) {
  ...
  const mountCurrent = (provider) => {
    ...
    try {
      ...
      disposeTool = ctx.tools.register(createDelegationTool(ctx, current, provider, current.models))
    } catch (error) {
  ...
  const reconcile = (value) => {
    ...
    if (provider === undefined) {
      ctx.logger.info(`subagent provider "${current.subagentProvider}" is not registered yet; ${current.toolName} will register when it appears`)
      return
    }
  ...
  ctx.systemPrompt.section({
    name: 'tool:subagent-dynamic-model:models',
    order: MODEL_SUBAGENT_SECTION_ORDER,
    text: (context) => disposeTool === undefined || ctx.tools.get(current.toolName, context.scope) === undefined
      ? ''
      : `For model-selectable delegation, use \`${current.toolName}\` and choose only from these configured routes. Treat each description as the owner's routing policy rather than guessing from the raw model name.\n${renderConfiguredModels(current.models)}`,
  })

  reconcile(settingsScope.get())
  ctx.effect(() => settingsScope.watch((next) => {
    reconcile(next)
  }), 'dsh-subagent-dynamic-model: settings watcher')
}
