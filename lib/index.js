...
function hasMatchingSettlementNotice(events, childId, stopReason) {
  const expected = expectedSettlementSummary(childId, stopReason)
  ...
  return events.some((event) => (
    ...
    && event.data.inserted.some((message) => (
      ...
      && message.source.summary === expected
    ))
  ))
}
...
function createOrchestrationTracker(ctx) {
  ...
  const updateRecord = (record, model, label, agent) => {
    ...
    if (record.agent === undefined && agent !== undefined) {
      record.agent = agent
      record.boundary = sessionEvents(agent.session).length
    }
  ...
  const createRecord = (state, childId, runId, model, label, agent, claimedByTool = false, parentBoundary) => {
    let resolve
    ...
    const record = {
      ...
      label,
      agent,
      boundary: agent === undefined ? 0 : sessionEvents(agent.session).length,
  ...
  const discoverResidentChildren = (state) => {
    ...
    for (const child of agents.list()) {
      ...
      const descriptor = sessionEvents(child.session)
        .findLast((event) => event.type === 'subagent/descriptor')?.data
      if (descriptor?.mode !== 'continuable') continue
      createRecord(state, child.id, undefined, descriptor.agentModel, descriptor.label, child)
    }
  }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    for (const state of states.values()) {
      if (state.waiting === 0 || state.parent.session !== session) continue
  ...
  const reconcile = (records) => {
    ...
    for (const record of records) {
      if (record.settlement !== undefined || record.runId === undefined) continue
      const current = agents.get(record.subagentId)
      if (record.agent === undefined) {
        updateRecord(record, undefined, undefined, current)
        continue
      }
      ...
      if (!hasMatchingSettlementNotice(parentEvents, record.subagentId, stopReason)) continue
      settle(record, {
        stopReason,
        lastAssistantMessage: recoveredAssistantOutput(events),
      })
      ctx.logger.warn(`${WAIT_TOOL_NAME}: recovered missed terminal event for subagent ${record.subagentId} run ${String(record.runId)}`)
    }
  }
  ...
  return {
    ...
    async wait(parent, signal) {
      ...
      const interruptionOutcome = () => ({
        kind: 'interrupted',
        pending: [...state.children.values()].map((record) => ({
          subagentId: record.subagentId,
