...
function requiredText(value, field, index) {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(`dsh-subagent-dynamic-model: models[${index}].${field} must be a non-empty string`)
  }
  return value.trim()
}
...
function normalizeConfiguredModels(input) {
  if (!Array.isArray(input)) {
    throw new Error('dsh-subagent-dynamic-model: models must be an array')
  }

  const aliases = new Set()
  return input.map((entry, index) => {
    if (typeof entry !== 'object' || entry === null || Array.isArray(entry)) {
      throw new Error(`dsh-subagent-dynamic-model: models[${index}] must be an object`)
    }

    const alias = requiredText(entry.alias, 'alias', index)
    if (!ALIAS_RE.test(alias)) {
      throw new Error(`dsh-subagent-dynamic-model: models[${index}].alias must use lowercase letters, numbers, underscores, or hyphens`)
    }
    if (aliases.has(alias)) {
      throw new Error(`dsh-subagent-dynamic-model: duplicate model alias "${alias}"`)
    }
    aliases.add(alias)
    ...
    if (!Array.isArray(sourceTags)) {
      throw new Error(`dsh-subagent-dynamic-model: models[${index}].tags must be an array`)
    }
    const tags = []
    ...
    for (let tagIndex = 0; tagIndex < sourceTags.length; tagIndex += 1) {
      ...
      if (!TAG_RE.test(tag)) {
        throw new Error(`dsh-subagent-dynamic-model: models[${index}].tags[${tagIndex}] must be lowercase kebab-case`)
      }
    ...
    if (maxTokens !== undefined && (!Number.isSafeInteger(maxTokens) || maxTokens <= 0)) {
      throw new Error(`dsh-subagent-dynamic-model: models[${index}].maxTokens must be a positive safe integer`)
    }
